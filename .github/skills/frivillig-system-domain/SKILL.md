...
description: Apply TimeWinder's authoritative Frivillig product boundaries and business rules. Use when designing, implementing, reviewing, or documenting volunteer identity, lifecycle, roles, teams, memberships, events, workplaces, opportunities, shift plans, shifts, staffing, qualifications, attendance, communication, CrewNet migration, or Operations Hub projections. Also use when deciding which product owns data or authorization in a Frivillig-related change.
---

# Frivillig System Domain

Protect the Frivillig product as the standalone source of truth for TimeWinder's volunteer and shift
domain. Treat technical reuse as subordinate to product ownership.
...
4. Keep Frivillig core functional when Operations Hub, CrewNet, and projection adapters are unavailable.
...
authoritative Frivillig model.

## Non-negotiable boundaries

- Make Frivillig the sole write master for volunteer identity/lifecycle, Frivillig authorization, teams,
  memberships, leadership, events, workplaces, opportunities, shift plans/types/shifts, applications,
  assignments, qualifications, attendance, shift communication, and Frivillig reporting.
- Let Operations Hub own only its add-on domains and local authorization. Consume Frivillig data through a
...
- Maintain one write owner per entity. Make projections idempotent, retryable, observable, and unable to
  roll back a successful Frivillig mutation.
- Separate Frivillig roles from Operations Hub roles. Never infer administrator or scoped access across
...
- Does a projection failure leave the Frivillig mutation committed and recoverable?
- Are CrewNet identifiers optional provenance rather than runtime keys?
- Does the UI live in Frivillig unless the feature is explicitly an add-on?
...
- [ ] Frivillig authorization resolves from Frivillig-owned identity, role, team, and membership stores.
- [ ] Operations Hub tables are projections or add-on data, never hidden Frivillig masters.
